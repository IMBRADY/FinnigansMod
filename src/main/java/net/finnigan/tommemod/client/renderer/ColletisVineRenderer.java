...
/**
 * Direct adaptation of GrappleHookRenderer's manual quad-strip-between-two-points technique, retextured
 * with colletischain (body) / colletisthorn (head) - same 16x16 dimensions as webchain/webhand, so the
 * quad-strip math/scale is reused unmodified.
 */
public class ColletisVineRenderer extends EntityRenderer<ColletisVineEntity> {

    private static final ResourceLocation CHAIN_TEXTURE = new ResourceLocation(TommeMod.MOD_ID, "textures/entity/colletischain.png");
    private static final ResourceLocation HEAD_TEXTURE = new ResourceLocation(TommeMod.MOD_ID, "textures/entity/colletisthorn.png");
    ...
    public ColletisVineRenderer(EntityRendererProvider.Context context) {
        super(context);
    }

    @Override
    public ResourceLocation getTextureLocation(ColletisVineEntity entity) {
        return HEAD_TEXTURE;
    }

    @Override
    public void render(ColletisVineEntity entity, float entityYaw, float partialTicks, PoseStack poseStack,
        ...
        renderHeadTip(poseStack, buffer, packedLight, end);

        super.render(entity, entityYaw, partialTicks, poseStack, buffer, packedLight);
    }

    @Override
    public boolean shouldRender(ColletisVineEntity entity, net.minecraft.client.renderer.culling.Frustum frustum,
    ...
    private void renderHeadTip(PoseStack poseStack, MultiBufferSource buffer, int packedLight, Vec3 end) {
        // The vine head sprite — a simple always-facing-camera billboard at the tip
        poseStack.pushPose();
        poseStack.translate(end.x, end.y, end.z);
        poseStack.scale(0.3F, 0.3F, 0.3F);

        VertexConsumer consumer = buffer.getBuffer(RenderType.entityCutout(HEAD_TEXTURE));
        Matrix4f matrix = poseStack.last().pose();
